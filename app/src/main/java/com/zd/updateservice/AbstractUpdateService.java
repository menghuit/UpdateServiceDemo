 ...
 * 创建就开始请求检查更新
 */
public abstract class AbstractUpdateService<T> extends Service implements Callable<T>, Dispatcher.Connection<T> {
    private static final String TAG = "UpdateService";

    public static final String ACTION_FORCE_CHECK = "action_force_check";

    public static final int MSG_REGISTER_CHECK_LISTENER = 1;
    public static final int MSG_UNREGISTER_CHECK_LISTENER = 2;
    ...
    protected final Binder mBinder = new UpdateBinder(this);
    ...
    private static class CheckTask<Result> extends AsyncTask<Void, Void, Result> {
        WeakReference<AbstractUpdateService> refer;

        CheckTask(AbstractUpdateService service){
            refer = new WeakReference<AbstractUpdateService>(service);
        }
    ...
    public class UpdateBinder extends Binder {
        WeakReference<AbstractUpdateService> service;

        UpdateBinder(AbstractUpdateService service) {
            this.service = new WeakReference<AbstractUpdateService>(service);
        }

        public void registerUpdateListener(CheckUpdateListener<T> lis) {
            service.get().mDispatcher.enqueue(lis);
        }

        public void unregisterUpdateListener(CheckUpdateListener<T> lis) {
            service.get().mDispatcher.remove(lis);
        }
    }
    ...
    private final static class InnerHandler extends Handler {
        ...
        InnerHandler(AbstractUpdateService service){
            this.service = new WeakReference<AbstractUpdateService>(service);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case MSG_REGISTER_CHECK_LISTENER:
                    service.get().mDispatcher.enqueue((CheckUpdateListener) msg.obj);
                    break;
                case MSG_UNREGISTER_CHECK_LISTENER:
                    service.get().mDispatcher.remove((CheckUpdateListener) msg.obj);
                    break;
                default:
                    break;
            }
